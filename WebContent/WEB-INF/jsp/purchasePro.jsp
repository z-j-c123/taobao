        ...
        <script>
            window.onload = function () {
              ...
              //显示商品信息
            	...
            	$.ajax({
            		url: "${pageContext.request.contextPath}/findListIdShoppingCart",
            		data: JSON.stringify({
            			arr: arr,
            		}),
            		type: "post",
            		...
            		success: function(result){
            			if(result.flag){
                			let arr = result.data;  
                			if(arr.length<=2){
                				$(".addAddress").css("top", "-1900px")
                			}else{                				
	                			$(".addAddress").css("top", -arr.length * 600+"px");
                			}
                			$(".newAddress").on("click",function(){
            	                $(".addAddress").css("top", -arr.length * 240+"px").css("opacity", 1);
            	            })
            	            $(".iconcha").on("click", function(){
            	                $(".addAddress").css("top", "-4000px").css("opacity", 0)
            	            })
                			let proStr = "";
                        	for(let i = 0; i < arr.length; i++){
                        		proStr += `<li><table class="table table-border table-hover">
                        			<tr><td width="5%"><input type="checkbox" class="goodsCheck" checked>
                        			<input type="text" value="\${arr[i].commoditydetailsId}" name="commodityDetailsId" class="hidden userId"></td>
                    				<td width="23%">
            		                    <img src="/images/\${arr[i].commodityPicture1}" width="50" height="50" alt="">
            		                    <div class="proName">\${arr[i].commodityName}</div>
                    				</td>
                    				<td width="15%"><div>\${arr[i].specifications}</div></td>
                    				<td width="18%"><span class="pro-price">\${arr[i].price}</span>￥</td>
                    				<td><div class="number">
            		                       <button type="button" class="sub">-</button>
            		                       <input type="text" class="value" name="orderCount" id="number" value="\${arr[i].count}">
            		                       <button type="button" class="add">+</button>
            		                   </div></td>
                    				<td><span class="subtotal">\${arr[i].count * arr[i].price}</span></td></tr></table></li>`;
                        	}
                        	proList.innerHTML = proStr;
                        	total()
